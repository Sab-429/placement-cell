  ...
  return (
    <div className="space-y-3">
      {items.map((item, i) => (
        <div
          key={i}
          className="flex items-start justify-between gap-3 bg-white
...
export default function StudentProfile() {
  ...
  const [workExperience, setWorkExperience] = useState(
    toItemList(cached?.work_experience),
  );
  ...
  const [certificates, setCertificates] = useState(
    toItemList(cached?.certificates),
  );
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        setLoading(true);
        const { data } = await client.get(`/student/students/${userId}`, {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/student/students/${userId}/resume/download`,
        {
  ...
  const handleResume = async (e) => {
    ...
    try {
      const { data } = await client.post(
        `/student/students/${userId}/resume`,
        fd,
      );
      const updated = {
        ...profile,
  ...
  const handleGenerate = async () => {
    try {
      ...
      const updated = { ...profile, resume_ready: false,  resume_source: 'generating', };
      localStorage.setItem(cacheKey, JSON.stringify(updated));
      setProfile(updated);
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
            <div className="relative">
              {photoSrc ? (
                ...
                <div
                  className="w-20 h-20 rounded-full bg-white/10 flex items-center
        ...
        {activeTab === "resume" && (
          <div className="space-y-4">
              ...
              {profile?.resume_ready ? (
                <div
                  className="flex items-center justify-between bg-green-50 border
                        border-green-200 rounded-xl px-5 py-4"
                >
                ...
                <div
                  className="flex items-center gap-3 bg-gray-50 border border-dashed
                        border-gray-300 rounded-xl px-5 py-4"
                >
                  <FileText className="w-5 h-5 text-gray-400" />
                  <div>
                    <p className="text-sm text-gray-500">No resume yet</p>
                    <p className="text-xs text-gray-400 mt-0.5">
                      Upload your own PDF or generate one from your profile
                      below
                    </p>
                  </div>
                </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <p className="font-semibold text-sm">Upload your PDF</p>
                ...
                <ul className="space-y-1 mb-4">
                  {[
                    ...
                    "Any format you already have",
                    "Instant — available right away",
                  ].map((item) => (
                    <li
                      key={item}
                ...
                <label className="cursor-pointer block">
                  <div
                    className="flex items-center justify-center gap-2 bg-blue-600 text-white
                          px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-blue-700
                          transition-colors"
                  >
              ...
              <div className="bg-white rounded-2xl border border-gray-200 p-6">
                ...
                <ul className="space-y-1 mb-4">
                  {[
                    "Built from your profile sections",
                    "Includes work, projects, education",
            ...
            <p className="text-xs text-gray-400 text-center">
              Both options save separately. Switching between them won't delete
              the other. Fill in Experience, Projects and Education tabs to get
              a complete generated resume.
            </p>
          </div>
        )}
        ...
        {activeTab === "skills" && (
          <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
            ...
            {domains.length > 0 ? (
              ...
              <div
                className="flex items-center justify-center py-10 bg-gray-50 rounded-xl
                              border border-dashed border-gray-200 text-gray-400 text-sm"
              >
            ...
            <ItemList
              ...
              renderItem={(item) => (
                <>
                  ...
                  {item.tech && (
                    <p className="text-xs text-blue-600 mt-0.5">{item.tech}</p>
                  )}
                  {item.description && (
                    <p className="text-xs text-gray-500 mt-1 leading-relaxed">
                      {item.description}
